...
export default function SimplePage({ slug: fixedSlug }) {
  ...
  return (
    <main className="container-pad py-14">
      <p className="text-sm font-semibold uppercase tracking-wide text-gold">{titles[slug] || "Page"}</p>
      ...
      {data.coreValues?.length > 0 && <div className="mt-10"><h2 className="text-2xl font-bold">Core values</h2><div className="mt-4 flex flex-wrap gap-2">{data.coreValues.map((v) => <span className="rounded-md bg-teal-50 px-3 py-2 text-sm font-medium text-brand" key={v}>{v}</span>)}</div></div>}
    </main>
  );
}
